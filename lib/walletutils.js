...
WalletUtils.publicKeyToAESKey = function(pubKey) {
  $.checkArgument(pubKey && _.isString(pubKey));
  $.checkArgument(Bitcore.PublicKey.isValid(pubKey), 'The public key received is invalid');
  var pk = Bitcore.PublicKey.fromString(pubKey);
  return Bitcore.crypto.Hash.ripemd160(Bitcore.crypto.Hash.sha256(pk.toBuffer())).slice(0, 16).toString('base64');
};
